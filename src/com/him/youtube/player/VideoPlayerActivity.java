...
//        getWindow().setFlags(android.view.WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON, android.view.WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);
		...
		mQueryYouTubeTask = (QueryYouTubeTask) new QueryYouTubeTask().execute(mVideoId);
	}
	...
	protected void initView() {
		...
		mVideoView.setOnPreparedListener( new MediaPlayer.OnPreparedListener() {
			...
			public void onPrepared(MediaPlayer pMp) {
				if(mQueryYouTubeTask != null && mQueryYouTubeTask.isCancelled())
					return;
...
//		getWindow().clearFlags(android.view.WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);
	...
	private class QueryYouTubeTask extends AsyncTask<String, String, Uri> {
		...
		protected Uri doInBackground(String... pParams) {
			String uriStr = null;
			int youTubeFmtQuality = YouTubeFMTQuality.GPP3_MEDIUM;   // 3gpp medium quality, which should be fast enough to view over EDGE connection
			
			if(isCancelled())
				return null;
			
			try {
				WifiManager lWifiManager = (WifiManager) VideoPlayerActivity.this.getSystemService(Context.WIFI_SERVICE);
				...
				uriStr = YouTubeAPI.getYouTubeUrl(youTubeFmtQuality, true, mVideoId);
		...
		protected void onPostExecute(Uri pResult) {
			...
			try {
				...
				final MediaController mediaController = new MediaController(VideoPlayerActivity.this);
